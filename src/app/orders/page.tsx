        ...
        {!orders || orders.length === 0 ? (
          <div className="text-center py-20">
            ...
            <div className="flex gap-3 justify-center">
              ...
              <Link href="/book/errand" className="inline-flex items-center gap-2 rounded-xl border-2 border-orange-500 px-5 py-2.5 text-sm font-semibold text-orange-500 hover:bg-orange-50 transition">
                🛒 Send Errandboi
              </Link>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {orders.map((order) => (
              <Link key={order.id} href={`/orders/${order.id}`}>
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md hover:border-orange-100 transition-all">
                  ...
                  {order.order_type === "ride" && (
                    <div className="mt-3 pt-3 border-t border-gray-50 text-sm text-gray-500">
                      <span className="font-medium text-gray-700">{order.pickup_address}</span> → {order.dropoff_address}
                    </div>
                  )}
